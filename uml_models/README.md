...
`DataRetriever` is modeled as the controlled access point for medical staff queries. Rather than letting every subsystem inspect storage directly, retrieval requests pass through a class that can apply access rules and validation. This is important because the CHMS stores sensitive patient information. Staff should only retrieve the records they are authorized to view, while alert generation should only access the measurement data it needs for safety checks.
